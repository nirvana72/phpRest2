...
class Medoo
{
     ...
     * @var bool
     */
    public $logging = false; // nij 修改此变量为外部可修改
    ...
    public function exec(string $statement, array $map = [], callable $callback = null): ?PDOStatement
    {
        ...
        // nij 记录结束时间
        $timeFinish = $this->msectime();
        // nij 把本条SQL耗时放入log数组第三位
    ...
    protected function generate(string $statement, array $map): string
    {
        ...
        foreach ($map as $key => $value) {
            if ($value[1] === PDO::PARAM_STR) {
                ...
                $replace = $this->quote(strval($value[0]));
     ...
     * nij 获取自动增长ID
     */
    public function getAutoId(string $name = null): int
    {
        $id = $this->pdo->lastInsertId($name);
        if ($id === false) throw new PDOException('get lastInsertId = false');
        return intval($id);
    }
